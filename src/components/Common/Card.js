...
export default function Card({ image, title, buttonText }) {
	...
	return (
		<div className="relative border shadow-lg shadow-gray-600/80 border-white [&>img]:hover:scale-110  lg:w-[30%] w-[45%]  lg:my-8 my-2 rounded-xl lg:h-[30%] h-[40%] flex overflow-hidden">
			<Image
				src={image}
				className="w-full h-full duration-500 ease-in-out object-contain"
				alt={`${title} Category image`}
			/>
			<Roll right>
				<div className="absolute flex w-full h-full flex-col top-0 left-0 justify-center items-center">
					<h1 className="lg:text-4xl text-sm mb-4 font-semibold text-white">
						{title}
					</h1>
					<button class="relative inline-block lg:text-xl text-xs group">
						<span
